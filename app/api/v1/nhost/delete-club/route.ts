...
export async function POST(req: NextRequest) {
    try {
        // 1. Verify Authentication & Authorization (Only high-level admins can delete clubs)
        const session = await verifySession(req, ['president', 'admin', 'developer']);
        if (!session) {
            const basicSession = await verifySession(req);
            if (!basicSession) {
                return unauthorizedResponse('Authentication required to delete clubs');
            }
            return forbiddenResponse('Only the President or Administrators can delete clubs');
        }
        ...
        console.error("Server error in delete-club:", err);
