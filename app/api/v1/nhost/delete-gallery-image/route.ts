...
export async function POST(req: Request) {
    try {
        ...
        if (!body.id) {
            return NextResponse.json({ message: 'Missing gallery image ID' }, { status: 400 });
        }
        ...
        const mutation = `
            mutation DeleteGalleryImage($id: uuid!) {
                delete_gallery_images_by_pk(id: $id) {
                    id
                }
            }
        `;
        ...
        console.error("Server error in delete-gallery-image:", err);
