...
export default function GirlsWardenDashboard() {
    ...
    const { isAuthorized, isLoading } = useDashboardAuth({
        allowedRoles: ['girls-warden', 'girls_warden', 'admin', 'developer', 'president']
    });

    // Filtering explicitly for Girls Hostel
    const girlsHostelTickets = useMemo(() => tickets.filter(t => t.hostelType === 'Girls Hostel'), [tickets]);

    const displayTickets = useMemo(() => {
        return girlsHostelTickets.filter(ticket => filterStatus === 'All' ? true : ticket.status === filterStatus);
    }, [girlsHostelTickets, filterStatus]);
    ...
    const stats = useMemo(() => {
        ...
        let inProgress = 0;
        let completed = 0;
        girlsHostelTickets.forEach(t => {
            if (t.status === 'Pending') pending++;
            else if (t.status === 'In Progress') inProgress++;
            else if (t.status === 'Completed') completed++;
        });
        return { pending, inProgress, completed, total: girlsHostelTickets.length };
    }, [girlsHostelTickets]);
    ...
    return (
        <div className="min-h-screen bg-[#0B0B14] text-white pt-24 pb-12 font-sans selection:bg-[#ec4899]/30">
            <div className="container mx-auto px-4 max-w-6xl">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <Badge variant="outline" className="border-[#ec4899]/30 text-[#ec4899] bg-[#ec4899]/10 rounded-full px-3 py-1 text-xs font-mono">
                                WARDEN PORTAL
                            </Badge>
                        </div>
                        ...
                        <p className="text-[#64748B] text-sm mt-3 font-mono">
                            Manage and resolve complaints reported in the Girls Hostel.
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="bg-[#111625] border border-[#ec4899]/20 rounded-2xl p-5 flex flex-col items-center justify-center text-center shadow-[0_0_15px_rgba(236,72,153,0.05)]">
                        <p className="text-[#ec4899] text-xs font-bold uppercase tracking-widest mb-2">In Progress</p>
                        <p className="text-3xl font-black text-[#ec4899]">{stats.inProgress}</p>
                    </div>
                ...
                <div className="space-y-4">
                    {displayTickets.length === 0 ? (
                        <div className="text-center py-20 bg-[#111625] rounded-3xl border border-white/5 shadow-xl">
                            ...
                            <p className="text-[#64748B]">No {filterStatus !== 'All' ? filterStatus.toLowerCase() : ''} complaints in the Girls Hostel.</p>
                        </div>
                    ) : (
                        <AnimatePresence>
                            {displayTickets.map((ticket, i) => (
                                ...
                                >
                                    <div className="bg-[#111625] border border-white/10 hover:border-[#ec4899]/30 rounded-2xl overflow-hidden shadow-lg transition-colors p-6">
                                        <div className="flex flex-col lg:flex-row gap-6">
                                            ...
                                            <div className="flex-1 space-y-4">
                                                <div className="flex flex-wrap items-center gap-2">
                                                    ...
                                                    <Badge className={`${
                                                        ...
                                                        ticket.priority === 'Medium' ? 'bg-[#ec4899]/20 text-[#ec4899] border-[#ec4899]/10' : 
                                                    ...
                                                    {ticket.roomNumber && (
                                                        <Badge variant="outline" className="border-pink-500/30 text-pink-500 bg-pink-500/5 rounded-md px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                                                ...
                                                {ticket.image && (
                                                    <div className="mt-4">
                                                        <div 
                                                            className="relative h-24 w-36 rounded-lg overflow-hidden border border-white/10 cursor-pointer group hover:border-[#ec4899]/50 transition-colors"
                                            ...
                                            <div className="flex flex-col gap-3 lg:w-48 xl:w-56 shrink-0 border-t lg:border-t-0 lg:border-l border-white/10 pt-4 lg:pt-0 lg:pl-6">
                                                <Badge variant="outline" className={`justify-center py-1 border font-bold tracking-widest text-[10px] uppercase w-full ${
                                                    ...
                                                    ticket.status === 'In Progress' ? 'bg-[#ec4899]/10 text-[#ec4899] border-[#ec4899]/30' : 
                                                        ...
                                                        <>
                                                            {ticket.status === 'Pending' && (
                                                                <Button 
                                                                    ...
                                                                    className="w-full bg-[#ec4899]/10 text-[#ec4899] hover:bg-[#ec4899]/20 border border-[#ec4899]/30 text-xs shadow-none"
                                                                >
                                                                    Mark In Progress
                                                                </Button>
                                                            )}
